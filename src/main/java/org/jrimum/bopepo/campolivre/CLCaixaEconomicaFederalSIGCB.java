...
class CLCaixaEconomicaFederalSIGCB extends AbstractCLCaixaEconomicaFederal {
	...
	CLCaixaEconomicaFederalSIGCB(Titulo titulo) {
		...
		StringBuffer novoNossoNumero = new StringBuffer(StringUtils.EMPTY);
		...
		if(conta.getCarteira().isComRegistro()){
			...
			novoNossoNumero.append(COBRANCA_REGISTRADA);
			
		}else{
			
			this.add(new Field<Integer>(COBRANCA_NAO_REGISTRADA, 1));
			novoNossoNumero.append(COBRANCA_NAO_REGISTRADA);
		}
		...
		novoNossoNumero.append(EMISSAO_CEDENTE);
		novoNossoNumero.append(nossoNumero);
		titulo.setNossoNumero(novoNossoNumero.toString());
